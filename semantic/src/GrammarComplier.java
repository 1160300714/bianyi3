		...
		while(pos<token_list.size()){
			...
			if(leftest.getName().equals("M68_2")){
				...
				if(inh==null || inh.equals("null"))
					...
					for(int i=code4.size()-1;i>=0;i--){
						if(code4.get(i)!=null&&code4.get(i).startsWith(temp)){
							code4.remove(i);
						}
					}
				}
				...
				code4.add("( "+"j="+","+b+","+"1"+","+(code4.size()+2)+" )");
				father.attribute.put("backpatch", ""+code4.size()); 
				code4.add(null);
				...
				code4.add(null);
				code4.set(backpatch, "( "+"j"+","+"--"+","+"--"+","+code4.size()+" )");//回填
				
				father.attribute.put("backpatch", ""+(codes.size()-1));
				father.attribute.put("backpatch", ""+(code4.size()-1));
				...
				father.attribute.put("backto", ""+code4.size());
			}
//			System.out.println(leftest.getName());
		...
		for(int i=0;i<codes.size();i++){
			System.out.println(i+"\t"+codes.get(i)+"\t"+code4.get(i));
		}
